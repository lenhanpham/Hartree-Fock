...
def OverLap_S_int(Alpha,Beta,R2_ab):
    ...
    """
    return (np.pi/(Alpha+Beta)**1.5)*np.exp((-Alpha*Beta/(Alpha+Beta)*R2_ab))
...
def Kinetic_T_int(Alpha,Beta,R2_ab):
    ...
    return Alpha*Beta/(Alpha+Beta)*(3.0-(2.0*Alpha*Beta/(Alpha+Beta)*R2_ab)*\
        np.pi/(Alpha+Beta)**1.5*np.exp(-Alpha*Beta*R2_ab/(Alpha+Beta)))
...
def Potential_V_int(Alpha,Beta,Zc,R2_ab,R2_pc):
    ...
    return -2.0*np.pi/(Alpha+Beta)*Zc*np.exp(-Alpha*Beta*R2_ab/(Alpha+Beta))*F0((Alpha+Beta)*R2_pc)
...
def TwoE_int(Alpha,Beta,Gamma,Delta,R2_ab,R2_cd,R2_pq):
    ...
    return 2.0*np.pi**2.5/((Alpha+Beta)*(Gamma+Delta)*np.sqrt(Alpha+Beta+Gamma+Delta))*\
           np.exp(-Alpha*Beta*R2_ab/(Alpha+Beta) - Gamma*Delta*R2_cd/(Gamma+Delta))*\
...
def AO_Integral(N,R,Zeta1,Zeta2,Z_H,Z_He):
    ...
    S12,T11,T12,T22,V11H,V12H,V22H,V11He,V12He,V22He,V1111,V2111,V2121,V2211,V2221,V2222 = \
    ...
    Coefft = np.array([[1.00000,0.0000000,0.000000],
                       [0.678914,0.430129,0.000000],
                       [0.444635,0.535328,0.154329]])

    Expont = np.array([[0.270950,0.000000,0.000000],
    ...
    for i in range(N):
        for j in range(N):
            ...
            S12 = S12 + OverLap_S_int(Exp_H1s[i],Exp_He1s[j],R2)*Coe_H1s[i]*Coe_He1s[j]
            T11 = T11 + Kinetic_T_int(Exp_H1s[i],Exp_H1s[j],0.0)*Coe_H1s[i]*Coe_H1s[j]
            T12 = T12 + Kinetic_T_int(Exp_H1s[i],Exp_He1s[j],R2)*Coe_H1s[i]*Coe_He1s[j]
            T22 = T22 + Kinetic_T_int(Exp_He1s[i],Exp_He1s[j],0.0)*Coe_He1s[i]*Coe_He1s[j]

            # Z_H and Z_He are atomic numbers of H and He provided from the input
            V11H  = V11H  + Potential_V_int(Exp_H1s[i],Exp_H1s[j],Z_H,0.0,0.00)*Coe_H1s[i]*Coe_H1s[j]
            V12H  = V12H  + Potential_V_int(Exp_H1s[i],Exp_He1s[j],Z_H,R2,R2_ap)*Coe_H1s[i]*Coe_He1s[j]
            V22H  = V22H  + Potential_V_int(Exp_He1s[i],Exp_He1s[j],Z_H,0.0,R2)*Coe_He1s[i]*Coe_He1s[j]

            V11He = V11He + Potential_V_int(Exp_H1s[i],Exp_H1s[j],Z_He,0.0,R2)*Coe_H1s[i]*Coe_H1s[j]
            V12He = V12He + Potential_V_int(Exp_H1s[i],Exp_He1s[j],Z_He,R2,R2_bp)*Coe_H1s[i]*Coe_He1s[j]
            V22He = V22He + Potential_V_int(Exp_He1s[i],Exp_He1s[j],Z_He,0.0,0.0)*Coe_He1s[i]*Coe_He1s[j]
        ...
        for j in range(N):
            for k in range(N):
                for l in range(N):
                    ...
                    V1111 = V1111 + TwoE_int(Exp_H1s[i],Exp_H1s[j],Exp_H1s[k],Exp_H1s[l],0.0,0.0,0.0)*Coe_H1s[i]*Coe_H1s[j]*Coe_H1s[k]*Coe_H1s[l]
                    V2111 = V2111 + TwoE_int(Exp_He1s[i],Exp_H1s[j],Exp_H1s[k],Exp_H1s[l],R2,0.0,R2_ap)*Coe_He1s[i]*Coe_H1s[j]*Coe_H1s[k]*Coe_H1s[l]
                    V2121 = V2121 + TwoE_int(Exp_He1s[i],Exp_H1s[j],Exp_He1s[k],Exp_H1s[l],R2,R2,R2_pq)*Coe_He1s[i]*Coe_H1s[j]*Coe_He1s[k]*Coe_H1s[l]
                    V2211 = V2211 + TwoE_int(Exp_He1s[i],Exp_He1s[j],Exp_H1s[k],Exp_H1s[l],0.0,0.0,R2)*Coe_He1s[i]*Coe_He1s[j]*Coe_H1s[k]*Coe_H1s[l]
                    V2221 = V2221 + TwoE_int(Exp_He1s[i],Exp_He1s[j],Exp_He1s[k],Exp_H1s[l],0.0,R2,R2_bq)*Coe_He1s[i]*Coe_He1s[j]*Coe_He1s[k]*Coe_H1s[l]
                    V2222 = V2222 + TwoE_int(Exp_He1s[i],Exp_He1s[j],Exp_He1s[k],Exp_He1s[l],0.0,0.0,0.0)*Coe_He1s[i]*Coe_He1s[j]*Coe_He1s[k]*Coe_He1s[l]
...
def Diag(Fprime):
    ...
    E[0,0] = Fprime[0,0]*math.cos(TheTa)**2 + Fprime[1,1]*math.sin(TheTa)**2 + Fprime[0,1]*math.sin(2.0*TheTa)
    E[1,1] = Fprime[1,1]*math.cos(TheTa)**2 + Fprime[0,0]*math.sin(TheTa)**2 + Fprime[0,1]*math.sin(2.0*TheTa)
    E[1,0] = 0.0
    E[0,1] = 0.0
...
def HFCALC(N, R, Zeta1, Zeta2, Z_H, Z_He):
    ...
    S12, T11, T12, T22, V11H, V12H, V22H, V11He, V12He, \
    V22He, V1111, V2111, V2121, V2211, V2221, V2222 = AO_Integral(N,R,Zeta1,Z_H,Z_H,Z_He)

    H, S, X, V = Collect(S12,T11,T12,T22,V11H,V12H,V22H,V11He,V12He,V22He,V1111,V2111,V2121,V2211,V2221,V2222)
